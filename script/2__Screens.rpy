            ...
            for index, card in enumerate(what):
                showif offset<=index<offset+(line_per_page*card_per_line):
                    ...
                    fixed at trs_insane_animation(end={"xalign":index2%card_per_line / (card_per_line-1), "ypos":int(index2/card_per_line) * int(333*zoom)}):
                        xsize int(g.card_xsize*zoom)-5
                        ysize int(g.card_ysize*zoom)-5 # i do -5 because otherwise you cant navigate using arrow keys, my guess is because of the int fucking some scaling
                    # fixed:
